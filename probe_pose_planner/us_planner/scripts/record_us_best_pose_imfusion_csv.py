...
"""
record_us_best_pose_imfusion_csv.py

Record geometry_msgs/PoseStamped messages from a topic (default: /us_best_pose) and export them
to a CSV file compatible with ImFusion.

For each incoming PoseStamped:
1) Transform the pose into a chosen base frame using TF2 (base_frame, typically "world").
2) Convert the pose into a 4x4 homogeneous transform in millimeters.
3) Apply a fixed calibration matrix to map from robot/base coordinates to ImFusion coordinates:
     T_imfusion_mm = T_IMFUSION_FROM_ROBOT @ T_robot_mm
4) Write one CSV row (no timestamp):
     x_mm, y_mm, z_mm, qx, qy, qz, qw

What you must change to test with your own setup:
- ~topic: the PoseStamped topic you want to record (absolute vs relative name matters)
- ~base_frame: the frame you want to express poses in (must be available in TF)
- ~output_csv: output path for the CSV file
- T_IMFUSION_FROM_ROBOT: update this 4x4 matrix to match your calibration (millimeters)
...
class Recorder:
    def __init__(self):
        rospy.init_node("record_us_best_pose_imfusion_csv", anonymous=False)

        self.topic = rospy.get_param("~topic", "/us_best_pose")
        self.base_frame = rospy.get_param("~base_frame", "world")
        self.output_csv = rospy.get_param("~output_csv", "/tmp/us_best_pose_imfusion.csv")
        ...
        rospy.loginfo(
            "Recording PoseStamped from '%s' to '%s' (base_frame='%s')",
            self.topic,
            self.output_csv,
            self.base_frame,
        )
    ...
    def cb(self, msg: PoseStamped):
        """Transform pose to base_frame, map to ImFusion coordinates, and write one CSV row."""
        try:
